...
EPOCHS = 10
BATCH_SIZE = 4096
AID_DATA_DIR = '/mnt/public/fangtongcheng/base_code_test/courses_proj/recsys/'
...
def train_xDeepFM_model_demo(device):
    ...
    # 初始化 TensorBoard
    writer = SummaryWriter(log_dir='runs/xDeepFM_experiment')
    ...
    for epoch in range(1, EPOCHS + 1):
        train_loss = train(xdeepfm, train_filelist, train_item_count, featIndex, feat_cnt, device, optimizer, epoch, writer)
        test_loss = test(xdeepfm, test_filelist, test_item_count, featIndex, feat_cnt, device, epoch, writer)
...
def test(model, test_filelist, test_item_count, featIndex, feat_cnt, device, epoch, writer=None):
    fname_idx = 0
    pred_y, true_y = [], []
    ...
    with torch.no_grad():
        ...
        roc_auc = roc_auc_score(y_true=np.array(true_y), y_score=np.array(pred_y))
...
def train(model, train_filelist, train_item_count, featIndex, feat_cnt, device, optimizer, epoch, use_reg_l2=True, writer=None):
    fname_idx = 0
    features_idxs, features_values, labels = None, None, None
    ...
    for batch_idx in range(math.ceil(train_item_count / BATCH_SIZE)):
        ...
        if writer:
            writer.add_scalar('Loss/train_batch', loss.item(), epoch * len(train_filelist) + batch_idx)
