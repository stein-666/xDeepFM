...
def train_model(model):
    ...
    for epoch in range(args.Epochs):
        ...
        for step, x in enumerate(train_loader):
            cat_fea, num_fea, label = x[0].cuda(), x[1].cuda(), x[2].cuda()
            if torch.cuda.is_available():
                cat_fea, num_fea, label = cat_fea.cuda(), num_fea.cuda(), label.cuda()
        ...
        if cur_auc > best_auc:
            ...
            torch.save(model.state_dict(), './save_model/xdeepfm.bin')
...
if __name__ == '__main__':
    ...
    model = xDeepFM(cat_fea_unique, num_fea_size=len(dense_features))
    ...
    with torch.no_grad():
        ...
        # print(cur_auc)
        # return cur_auc
    print(f"average auc score = {avg_auc/total}")
